...
def add_rank_mean_and_group_columns(data: pd.DataFrame) -> pd.DataFrame:
    ...
    for rank_count, return_horizon in PAST_RETURN_COMBOS:
        ...
        if Config.RANK_VOL_REQUIRE_FULL_WINDOW:
            # 与 rank_vol 保持一致：严格模式下，m 个排名必须全部非空才计算均值。
            full_window = ranks.notna().all(axis=1)
            # 先把整列设成 NaN，再只给完整窗口填值，这样不完整窗口天然保持缺失。
            result[rank_mean_column] = np.nan
            result.loc[full_window, rank_mean_column] = ranks.loc[
                full_window
            ].mean(axis=1)
        else:
            # 宽松模式下沿用 pandas 的 skipna 均值；只有 m 个排名全缺失时结果才是 NaN。
            result[rank_mean_column] = ranks.mean(axis=1, skipna=True)

        result[top_half_column] = np.nan
        non_missing_rank_mean = result[rank_mean_column].notna()
        ...
        cross_section_rank_pct = result.groupby(group_columns, sort=False)[
            rank_mean_column
        ].rank(method="average", pct=True)
        # 只给排名均值非缺失的行打 -1/0/1；排名均值是 NaN 的行，分组标记也必须保持 NaN。
        result.loc[non_missing_rank_mean, top_half_column] = 0
        result.loc[
...
def validate_grouping_factors(original: pd.DataFrame, result: pd.DataFrame) -> None:
    ...
    for rank_count, return_horizon in PAST_RETURN_COMBOS:
        ...
        actual_after_fac = list(result.columns[fac_index + 1 : fac_index + 4])
        ...
        # 排名均值缺失的行不应被强行分到 0 组，否则会污染后续分组统计。
...
def generate_grouping_factors(
    ...
    # 2. 计算本脚本新增的 rank_mean 和 is_top_half 两类列。
...
def print_summary(summary: dict[str, object]) -> None:
    ...
    # 行列数是最直接的完整性信号：行数不该变，列数应增加 10 列。
